...
//                      FIRMWARE VERSION
// =================================================================
const String FIRMWARE_VERSION = "V1.3.4-ec-fix";
...
// --- Reset-Logik mit Keypad-Bestätigung ---
  ...
  if (isSumUpTransactionActive) {
    ...
    if (now - sumUpStartTime > sumupTimeout) {
        ...
        currentSumUpTxId = "";
        displayErrorMessage("Zeit", "abgelaufen!");
    }
    ...
    // 2. STATUS PRÜFEN (Alle 2 Sekunden)
        ...
        int secondsLeft = (sumupTimeout - (now - sumUpStartTime)) / 1000;
        // Zeichne nur den Timer-Bereich neu, um Flackern zu minimieren
        tft.fillRect(200, 200, 60, 25, ILI9341_BLACK);
        tft.setCursor(200, 200);
        tft.setTextColor(ILI9341_WHITE);
        tft.setFont(&Poppins_Regular10pt7b);
        tft.print(secondsLeft); tft.println("s");

        // User Abort via '*' Taste ermöglichen
        char key = manualGetKeyState();
        if (key == '*') {
...
void handleSumUpPaymentInitiation() {
  ...
  if (selectedSlot == -1) {
    displayErrorMessage("Fach waehlen!", "Zuerst Nummer");
    return;
  }
  ...
  tft.setFont(&Poppins_Regular10pt7b);
  tft.setCursor(10, 80);
  tft.println("Terminal bereit...");
  
  tft.setCursor(10, 110);
  tft.setTextColor(ILI9341_WHITE);
  tft.print("Preis: "); tft.print(price, 2); tft.println(" EUR");
  
  if (credit > 0) {
    tft.setCursor(10, 135);
    tft.setTextColor(ILI9341_GREEN);
    tft.print("- Guthaben: "); tft.print(credit, 2); tft.println(" EUR");
  }

  tft.setCursor(10, 160);
  tft.setTextColor(ILI9341_ORANGE);
  tft.print("Zu zahlen: "); tft.print(remainingAmount, 2); tft.println(" EUR");
